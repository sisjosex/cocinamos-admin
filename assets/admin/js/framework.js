    ...
    this.init = function () {
        ...
        if( self.map === undefined ) {
            ...
            if(section == 'detail') {
                ...
                self.field_key = field.id ? field.id : section + '_' + model.table_name + '_' + field_id;
            }
            var field_name = field.id ? field.id : model.table_name + '[' + field_id + ']';
...
containers.form = function (model, container, module) {
    ...
    this.init = function () {
        ...
        if (container.form.settings.validation) {
            ...
            if(validation[model.table_name] != undefined) {

                self.validation_params = validation[model.table_name];
        ...
        for (var i in self.fields) {
            ...
            if (field.value) {
                ...
                if(field.type == 'dropdown') {
                    ...
                    addInline.attr('rel', field.model.table_name);
                    addInline.attr('key', field.key);
                    //addInline.attr('url', field.model.base_url + field.model.table_name + '/getNew');
            ...
            if (response[model.table_name][i] !== undefined) {
                ...
                if (element.is('input') || element.is('textarea')) {
                    ...
                    if(element.is('input:checkbox')) {
                        element.val('1');
                        if(response[model.table_name][i] == '1') {
    ...
    this.remoteLoad = function (url, data) {
        ...
        ajax({
            url: url ? url : BASE_URL + model.base_url + model.table_name + '/getNew',
            data: data,
            success: function (response) {
...
containers.detail = function (model, container, module) {
    ...
    this.init = function () {
        ...
        for (var key in model.field_group.form) {
            ...
            var field_id = field.id ? field.id : field.key;
            var field_key = field.id ? field.id : 'detail_' + model.table_name + '_' + field_id;
            var field_name = field.id ? field.id : model.table_name + '[' + field_id + ']';
            ...
            if (field.type === 'hidden' || (field.form_template)) {
                ...
                //self.validation_params = initFormField(self.validation_params, field, field_element, {field_id: field_id, field_name: field_name, field_key: field_key, read_only: true});
            ...
            if (response[model.table_name][i] !== undefined) {
                ...
                if (model.fields[i].type == 'dropdown' || model.fields[i].type == 'multiple' || model.fields[i].type == 'composite') {
                    ...
                    if( model.fields[i].type != 'map' ) {
                        ...
                        element.html(response[model.table_name][i]);
    ...
    this.actionRow = function (type, id, url, module_name, key, callback) {
        ...
        if (!url) {
            ...
            if (type === 'new') {
                ...
                module.containers.modal.element.find('.modal-header .title').text(lang('New') + ' ' + lang(model.table_name));
                ...
                module.containers.modal.element.find('.modal-header .title').text(lang('Edit') + ' ' +  lang(model.table_name));
                ...
                module.containers.modal.element.find('.modal-header .title').text(lang('Detail') + ' ' + lang(model.table_name));
        ...
        ajax({
            ...
            success: function (response) {
                ...
                if (response.message) {
                    ...
                    var cancel = buildFromTemplate('', 'button');
                    var group = buildFromTemplate('', 'field_footer');
                    cancel.text(lang('Aceptar'));
                    ...
                    group.append(cancel);
                    module.containers.modal.element.find('.modal-content').append(group);*/
                    ...
                    module.containers.form.buttons.submit.on('click', module.containers.form.saveCustom);*/
...
containers.toolbar = function (model, container, module) {
            ...
            for (var i in container.toolbar.settings.actions) {
                ...
                if (config.type == 'button') {
                    ...
                    if (config.action === 'new') {
                        ...
                        btn.attr('url', model.table_name + '/download/' + config.format + '/');
                        btn.attr('method', 'GET');
                        btn.on('click', self.actionButton);
                ...
                success: function(response) {
                    ...
                    for(var i in self.fields) {
                        ...
                        if (model.fields[i].type == 'dropdown' || model.fields[i].type == 'multiple' || model.fields[i].type == 'composite') {
                            ...
                            if(response[model.table_name] != undefined && response[model.table_name].dropdown && response[model.table_name].dropdown[i]) {
    ...
    this.loadDepends = function (event) {
        ...
        ajax({
            url: BASE_URL + model.base_url + model.table_name + '/loadDependsToolbar',
            data: data,
            success: function (response) {
        ...
        for (var i in module.model.fields) {
            ...
            if (field.type)
                if (field.type === 'inline') {
                            ...
                            toolbar: {
                                settings: {
                                    actions: [
                                        {
                                            type: 'button',
                                            title: (lang('New')+" "+ lang(field.model.table_name)),
                                            action: 'new'
                                        }
                                    ]
                                }
                    ...
                    console.log(module.containers.form.fields[i]);
