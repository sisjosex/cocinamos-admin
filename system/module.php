...
class Module extends Model
{
    ...
    function getList($count_only = FALSE, $row_only = FALSE)
    {
        ...
        if (!empty($this->filters)) {
            ...
            foreach ($this->filters as $key => $value) {
                ...
                $key = str_replace($this->table_name, $this->phisical_table, $key);
        ...
        if ( isset($_REQUEST['filters']) ) {
            ...
            foreach($_REQUEST['filters'] as $key => $val) {
                ...
                $key = str_replace($this->table_name.'_', $this->phisical_table.'.', $key);
                $key = str_replace($this->table_name, $this->phisical_table, $key);
    ...
    function loadDependsToolbar() {
        ...
        //$id = $_REQUEST[$this->table_name.'.id'];
        $data = array();
        foreach($_REQUEST as $key_ => $row) {

            if(strpos($key_, $this->table_name) !== false) {
                $data[str_replace(']', '', str_replace('[', '', str_replace($this->table_name.'_', '', $key_)))] = $row;
            }
        }
        $depending = $this->fields[$key]['key_depending'];
        ...
        foreach($depending as $column) {
            ...
            if( $field['type'] == 'composite' || $field['type'] == 'dropdown' || isset($field['depends_of']) ) {
                ...
                if($result && ($field['type'] == 'composite' || $field['type'] == 'dropdown') )  {
                    ...
                    $response[$this->table_name][$column] = '';

                    $response[$this->table_name]['dropdown'][$column] = $result;

                } else {

                    $response[$this->table_name][$column] = $result;
                }
    ...
    function saveInlineRelations($parent_id, $parentModel, $model, $values) {
        ...
        if(!empty($values)) {
            ...
            foreach($values as &$value) {
                ...
                if (isset($value[$this->table_name . '_id'])) {
                    $value[$this->phisical_table . '_id'] = $value[$this->table_name . '_id'];
                    unset($value[$this->table_name . '_id']);
                }
            }
